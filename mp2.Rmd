...
candidates_2012_house %>%
  left_join(committees_contributions, by = "fec_id") %>%
  group_by(candidate_name,cand_party_affiliation,cand_city,cand_office_district, fec_id, state, Total_primary, Total_general,percent, total= Total_primary + Total_general)%>%
summarise(transction=sum(transaction_amt)) %>%
filter(cand_party_affiliation == "DEM"|cand_party_affiliation == "REP") %>%
filter(percent > 1.5)

mp2_State<-function(state_a){
 candidates_2012_house %>%
    filter(state == state_a)}

mp2_plot<- function(data_arg){print(ggplot(data_arg,aes(x = total, y = transaction))+
  geom_point(aes(color = cand_party_affiliation , size = percent )) +
   scale_color_manual(values = c("D" = "darkblue", "R" = "red")) + 
    ggtitle(data_arg$state))}
